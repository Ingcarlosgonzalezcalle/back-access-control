...
const ProductForTransfer = async (body) => {
  ...
  try {
    ...
    const include = [
      {
        model: Store,
        attributes: ['id','name', 'type', 'productId']
      },
      {
        model: Product,
    ...
    const report = await Balance.findAll({
     ...
     const formattedReport = report.map(row => ({
            ...
            type: row.store?.type||'0',
            productStore: row.store?.productId||'0',
...
export default ProductForTransfer